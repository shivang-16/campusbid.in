...
const ShareSomething = ({
  ...
  const scrollContainer = (direction: number) => {
    const container = document.querySelector(
      ".relative.w-full.h-full.overflow-x-auto"
    ) as HTMLElement;
    if (container) {
  ...
  const handleCreatePost = async () => {
    try {
      ...
      // Fetch signed URLs
      const UrlsResponse = (await getSignedUrls(formattedFiles)) as {
        files: { putUrl: string }[];
      };
      ...
      const formattedData = {
        ...
        type: "post",
        isComment: post ? true : false,
        refPost: post ? post._id : null
      };
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full mx-auto rounded-3xl bg-[#f4ecd3] p-4 ${
        isModal ? "fixed max-w-[650px]" : "sticky bottom-2 "
      }`}
    >
      {post && (
        <div className="flex">
          ...
          <div className="flex flex-col ml-2 pb-8">
      ...
      <div className="flex gap-2">
        ...
        <div
          className={`flex flex-col gap-3 font-medium  w-full  rounded-3xl shadow-sm px-2 `}
          onDrop={handleDrop}
        ...
        >
          ...
          <div
            ...
            className={`flex ${
              isExpanded ? "pt-3 items-start" : "items-center"
            } gap-3 cursor-pointer ${isModal ? "h-[100px]" : ""}`}
          >
            {/* <textarea
              ...
              {files.length === 2 && (
                <div className="flex gap-3">
                  {files.map((file, index) => (
                    <div
                      key={index}
                      className="relative flex-1 h-64 bg-gray-100 border border-gray-300 shadow-sm rounded-lg overflow-hidden"
                    >
